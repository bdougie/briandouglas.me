...
In a world where [the principals are filling up their bags](https://x.com/GeoffreyHuntley/status/2009883461474205802), you need a superintendent.
...
You can't reproduce the failure. Even if you had logs, Ralph's nondeterminism means running the same prompt won't give you the same result. Without a complete conversation record, you're guessing at what actually happened.
...
The solution exists, we just haven't applied it yet. We need infrastructure that captures every conversation, stores it immutably, and lets you replay any session exactly as it happened.
